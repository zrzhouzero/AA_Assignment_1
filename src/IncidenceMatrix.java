 ...
 * updated by Zéro & Anna 14 April 2019
 */
public class IncidenceMatrix extends AbstractAssocGraph {
    ...
    public void removeVertex(String vertLabel) {
        ...
        // remove the vertex
        vertexLabel[index] = null;
        for (int i = 0; i < edgePointer; i++) {
            this.weightMatrix[index][i] = 0;
        }

        // remove the related edges
        for (int i = 0; i < edgePointer; i++) {
            if (edgeLabel[i] == null) continue;
            if (edgeLabel[i].getSrc().equals(vertLabel) || edgeLabel[i].getTar().equals(vertLabel)) {
                edgeLabel[i] = null;
    ...
    public void printEdges(PrintWriter os) {
        ...
        for (int i = 0; i < edgeLabel.length; i++) {
            if (this.edgeLabel[i] == null) continue;
            int index = findLabelIndexByLabel(edgeLabel[i].getSrc());
            if (this.weightMatrix[index][i] == 0) continue;
    ...
    private int findLabelIndexByLabel(String str) {
        ...
        for (int i = 0; i < this.vertexPointer; i++) {
            if (this.vertexLabel[i] == null) continue;
