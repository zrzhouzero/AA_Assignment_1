...
public class GraphEval {
    ...
    public static void processOperations(BufferedReader inReader, AssociationGraph graph,
        ...
        while (!bQuit && (line = inReader.readLine()) != null) {
            ...
            try {
                ...
                switch (command.toUpperCase()) {
                    ...
                    // remove vertex
                    case "RV":
                        long removeVertexStartTime = System.nanoTime();
                        if (tokens.length == 2) {
                            graph.removeVertex(tokens[1]);
                        } else {
                            System.err.println(lineNum + ": incorrect number of tokens.");
                        }
                        long removeVertexEndTime = System.nanoTime();
                        removeVertexTime += removeVertexEndTime - removeVertexStartTime;
                        break;
                    // k-nearest out-neighbourhood
                    case "ON":
                        long getOnStartTime = System.nanoTime();
                        if (tokens.length == 3) {
                        ...
                        long getOnEndTime = System.nanoTime();
                        getOutTime += getOnEndTime - getOnStartTime;
                        break;
                    // k-nearest in-neighbourhood
                    case "IN":
                        long getInStartTime = System.nanoTime();
                        if (tokens.length == 3) {
                        ...
                        long getInEndTime = System.nanoTime();
                        getInTime += getInEndTime - getInStartTime;
                        break;
                    // print vertices
                    case "PV":
        ...
        System.out.println("remove time: " + removeVertexTime);
        System.out.println("get in time: " + getInTime);
        System.out.println("get out time: " +getOutTime);
